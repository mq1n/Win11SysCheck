...
#define NOTIFY_OPACITY				0.0f		// 0-1 Toast opacity
...
#define NOTIFY_TOAST_FLAGS			ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoInputs   | ImGuiWindowFlags_NoFocusOnAppearing
	...
	inline void render()
	{
		...
		for (auto i = 0; i < toast_list.size(); i++)
		{
			...
			// Here we render the toast content
			{
				...
				{
					sprintf_s(buffer, "%s  %s zzz", current_toast->get_icon(), current_toast->content);
				}
				else
				{
